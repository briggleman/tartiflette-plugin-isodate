...
async def test_isodate_no_microseconds(mock_time_altzone):
    ...
    engine = await create_engine(
        sdl="""
        type ISODate {
            date: String @isoDate(microseconds: false, utc: false)
        }
        type Query {
            createdAt: ISODate
        }
...
async def test_isodate_no_timezone():
    ...
    engine = await create_engine(
        sdl="""
        type ISODate {
            date: String @isoDate(timezone: false, utc: false)
        }
        type Query {
            createdAt: ISODate
        }
...
async def test_isodate_no_microseconds_timezone():
    ...
    engine = await create_engine(
        sdl="""
        type ISODate {
            date: String @isoDate(microseconds: false, timezone: false, utc: false)
        }
        type Query {
            createdAt: ISODate
        }
